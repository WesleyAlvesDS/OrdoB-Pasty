...
export function SendTextToPc() {
  useEffect(() => {
    document.title = 'Envie texto do celular para o PC | Pasty'
    window.scrollTo(0, 0)
  }, [])
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-50 dark:bg-gray-950">
          ...
          {steps.map(({ step, title, desc, gradient }, index) => (
            ...
            >
              ...
              {index < steps.length - 1 && (
                <div className="hidden sm:block absolute top-8 left-[60%] w-[calc(80%)] h-0.5 bg-gradient-to-r from-violet-200 to-purple-200 dark:from-violet-800 dark:to-purple-800" />
              )}
              ...
              <div className="relative p-6 rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-sm transition-all duration-300 hover:shadow-xl hover:shadow-violet-100/50 dark:hover:shadow-violet-950/30 hover:-translate-y-1 hover:border-violet-200 dark:hover:border-violet-700">
                <div className={`w-14 h-14 rounded-xl bg-gradient-to-br ${gradient} flex items-center justify-center text-white text-lg font-bold shadow-lg mb-4 transition-transform duration-300 group-hover:scale-110`}>
                  {step}
                </div>
      ...
      {/* ─── Benefits ─────────────────────────────────── */}
      <section className="border-t border-gray-200 dark:border-gray-800 bg-gradient-to-b from-white to-violet-50/50 dark:from-gray-950 dark:to-violet-950/20">
        <div className="max-w-4xl mx-auto px-4 py-16">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {benefits.map(({ icon, title, desc }, index) => (
              ...
              >
                <span className="text-2xl flex-shrink-0 transition-transform duration-300 group-hover:scale-125">
                  {icon}
                </span>
                <div>
